...
wget --no-check-certificate -c -q -O /tmp/adss/adwhitelist https://gitcode.net/clion007/adss/raw/master/rules/diy/adwhitelist
...
while read -r line
do
	if [ -s "/etc/dnsmasq.d/adss/dnsAd" ]; then 
		sed -i "/$line/d" /etc/dnsmasq.d/adss/dnsAd
	fi
