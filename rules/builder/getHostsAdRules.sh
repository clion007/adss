...
cat /tmp/fastgithub.conf /tmp/someonewhocares.conf /tmp/adaway.conf > /tmp/hostsAd
echo
echo -e "\e[1;36m 删除hosts临时文件\e[0m"
rm -rf /tmp/fastgithub.conf /tmp/someonewhocares.conf /tmp/adaway.conf
echo
echo -e "\e[1;36m 删除注释和本地规则\e[0m"
sed -i '/#<localhost/,/#<\/localhost>/d' /tmp/hostsAd
sed -i '/local/d' /tmp/hostsAd
