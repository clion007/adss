...
wget --no-check-certificate -c -t 5 -T 60 -O /tmp/vokins.conf https://raw.githubusercontent.com/vokins/yhosts/master/dnsmasq/union.conf
echo
echo -e "\e[1;36m 下载yoyoAd广告规则\e[0m"
wget --no-check-certificate -c -t 5 -T 60 -O /tmp/yoyoAd.conf https://pgl.yoyo.org/adservers/serverlist.php?hostformat=dnsmasq\&showintro=0\&mimetype=plaintext
echo
echo -e "\e[1;36m 下载notrackAd广告规则\e[0m"
wget --no-check-certificate -c -t 5 -T 60 -O /tmp/notrackAdDomain.conf https://raw.githubusercontent.com/notracking/hosts-blocklists/master/domains.txt
echo
echo -e "\e[1;36m 下载Anti-Ad广告规则\e[0m"
wget --no-check-certificate -c -t 5 -T 60 -O /tmp/antiAd.conf https://gitee.com/privacy-protection-tools/anti-ad/raw/master/anti-ad-for-dnsmasq.conf
sed -i "/#/d" /tmp/antiAd.conf
sed -i 's/$/&127.0.0.1/g' /tmp/antiAd.conf
echo
sleep 3
echo -e "\e[1;36m 创建广告黑名单缓存\e[0m"
wget --no-check-certificate -c -t 5 -T 60 -O /tmp/adblacklist https://raw.githubusercontent.com/clion007/dnsmasq/master/adblacklist
