 #!/bin/sh
mkdir -p /tmp/adss
curl https://raw.gitmirror.com/clion007/adss/master/installer/copyright.sh -sLSo /tmp/adss/copyright.sh
...
echo 
curl https://raw.gitmirror.com/clion007/adss/master/updater/update.sh -sLSo /tmp/adss/update.sh
curl https://raw.gitmirror.com/clion007/adss/master/updater/rules_update.sh -sLSo /tmp/adss/rules_update.sh
if [ -s "/tmp/adss/update.sh" -a -s "/tmp/adss/rules_update.sh" ]; then
	if ( ! cmp -s /tmp/adss/update.sh /usr/share/adss/update.sh ); then
		...
		echo " `date +'%Y-%m-%d %H:%M:%S'`: 检测到新版规则升级脚本......开始更新！"
		echo 
