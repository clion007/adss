...
if [ -s "/tmp/adss/update.sh" -a -s "/tmp/adss/rules_update.sh" ]; then
	if ( ! cmp -s /tmp/adss/update.. /usr/share/adss/update.sh ); then
		...
		mv -f /tmp/adss/update.. /usr/share/adss/update.sh
		echo 
		. /usr/share/adss/update.sh
		echo " `date +'%Y-%m-%d %H:%M:%S'`: 升级脚本更新完成。"
		echo 
	elif ( ! cmp -s /tmp/adss/rules_update.. /usr/share/adss/rules_update.sh ); then
		echo " `date +'%Y-%m-%d %H:%M:%S'`: 检测到新版规则升级脚本......3秒后即将开始更新！"
		echo 
		sleep 3
		echo -e "\e[1;36m 开始更新规则升级脚本\e[0m"
		mv -f /tmp/adss/rules_update.. /usr/share/adss/rules_update.sh
		echo 
