...
if [ ! $? -eq 0 ]; then
  ...
  opkg list_installed | grep "dnsmasq" > /dev/null
  if [ ! $? -eq 0 ]; then
    message r "dnsmasq-full安装失败,请web登录路由器到系统软件包中手动安装后再试!"
    exit 1
  fi
fi
message l "倚赖关系处理完成"
...
download "config/prepare.sh" "${TMP_DIR}/prepare.sh"
. ${TMP_DIR}/prepare.sh

download "config/configer.sh" "${TMP_DIR}/configer.sh"
. ${TMP_DIR}/configer.sh
