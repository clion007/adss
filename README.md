...
1. 项目整体搬迁到gitcode，对安装脚本进行了优化与精简，只保留两个选项的脚本。
2. 优化修复了脚本规则生成中有时会导至没有写入规则的bug！
3. 广告规则源的优化，移除一些已经停止维护的规则，加入新的比较好的规则源。
4. 网络环境检测优化，监听局域网加入对IPv6的支持。
5. 优化上游DNS配置文件。
6. 自动修复新的openwrt固件中，因为植入ujail沙箱环境，限制dnsmasq对配置文件访问导至dnsmasq无法启动而无法解析域名的问题。
7. 优化了卸载脚本及定时自动维护脚本。
