...
download_base_rules() {
    ...
    # 下载DNS规则
    curl -s --connect-timeout 10 --retry 3 ${GH_PROXY_PREFIX}https://raw.githubusercontent.com/clion007/adss/master/rules/file/dnsrules.conf -o $TEMP_DIR/base_dnsrules.conf
    if [ $? -ne 0 ]; then
        log "基础DNS规则下载失败，尝试备用源"
        curl -s --connect-timeout 10 --retry 3 https://raw.githubusercontent.com/clion007/adss/master/rules/file/dnsrules.conf -o $TEMP_DIR/base_dnsrules.conf
    fi
    
    # 下载Hosts规则
    curl -s --connect-timeout 10 --retry 3 ${GH_PROXY_PREFIX}https://raw.githubusercontent.com/clion007/adss/master/rules/file/hostsrules.conf -o $TEMP_DIR/base_hostsrules.conf
    if [ $? -ne 0 ]; then
        log "基础Hosts规则下载失败，尝试备用源"
        curl -s --connect-timeout 10 --retry 3 https://raw.githubusercontent.com/clion007/adss/master/rules/file/hostsrules.conf -o $TEMP_DIR/base_hostsrules.conf
    fi
