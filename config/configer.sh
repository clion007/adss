...
echo 
echo "# 指定上游DNS服务器配置文件
resolv-file=/tmp/dnsmasq.d/adss/resolv-adss.conf

# 添加解析规则
conf-file=/tmp/dnsmasq.d/adss/rules/dnsrules.conf

# 添加额外hosts规则
addn-hosts=/tmp/dnsmasq.d/adss/rules/hostsrules.conf" > /tmp/dnsmasq.d/dnsmasq-adss.conf 
...
nameserver 205.252.144.228   # 香港 DNS IPv4" > /tmp/dnsmasq.d/adss/resolv-adss.conf 
if [ ! -f /tmp/dnsmasq.d/adss/rules/rules/userlist ]; then
	echo -e "\e[1;36m 创建自定义dnsmasq规则\e[0m"
	echo 
	echo "# 格式示例如下，删除address前 # 有效，添加自定义规则
# 后面的ip表示希望域名解析到的IP
# address=/telegram.org/149.154.167.99" > /tmp/dnsmasq.d/adss/rules/userlist 
fi
if [ ! -f /tmp/dnsmasq.d/adss/rules/rules/userblacklist ]; then
	echo -e "\e[1;36m 创建自定义广告黑名单\e[0m"
	echo 
	echo "# 请在下面添加广告黑名单
# 每行输入要屏蔽广告网址域名不含http://符号，如：www.baidu.com
# 支持不完整域名地址，支持通配符" > /tmp/dnsmasq.d/adss/rules/userblacklist 

fi	
if [ ! -f /tmp/dnsmasq.d/adss/rules/userwhitelist ]; then
